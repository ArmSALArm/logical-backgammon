...
function RuleAmLongBackgammon() {
     ...
     * @type {MoveActionType[]}
     */
    this.startAllowedCombinations = [3, 4, 6];
}
...
/**
 * Check is are allowed combination or not
 * @param dice
 * @returns bool
 */
// RuleAmLongBackgammon.prototype.isAllowedDiceCombinations = function(dice) {
//     if(model.Dice.isDouble()){
//         return ((this.startAllowedCombinations.indexOf(dice.values[0])) > -1);
//     }
//
//     return null;
// };
// RuleAmLongBackgammon.prototype.isLastTurnOfInitialPosition = function() {
//     return false;
// };
 ...
 *
 * @memberOf RuleAmLongBackgammon
 * @param {Match} currentGame - Match
 * @param {Piece} piece - Piece to move
 * @param {PieceType} type - Type of piece
 * @param {number} steps - Number of steps to increment towards first home position
 * @returns {MoveAction[]} - List of actions if move is allowed, empty list otherwise.
 */
RuleAmLongBackgammon.prototype.getMoveActions = function(currentGame, piece, steps) {
    var actionList = [];
    ...
    try {
        ...
        if (this.allPiecesAreHome(state, piece.type)) {
          ...
          /*
           ...
           - Opponent has no pieces there
                ...
                -piece is block last place of side and not allow opponent go to finish
                    Cases:
                    -opponent have a piece('s) which are no passed blocked side --> point is blocked, cannot place piece there
           ...
           */
            ...
            // Make sure the piece is not a second piece of Start place
            // If that second piece of start place but it is allowed combinations of Dice
            // if(RuleAmLongBackgammon.isAllowedDiceCombinations(currentGame.turnDice) || ! RuleAmLongBackgammon.isLastTurnOfInitialPosition()){
            // if(false){
                ...
                if ((destination >= 0) && (destination <= 23)) {
                    ...
                    var quarter = +(destination/6).toLocaleString()[0] + 1;
                    var maxOfQuarter = quarter * 6 - 1 ;
                    var minOfQuarter = maxOfQuarter - 5;
                    var countOfSamePieces = 0;
                    for(var i = minOfQuarter; i <= maxOfQuarter; i++ ) {
                        if(i !== destination) {
                            var destTopPieceCurrent = model.State.getTopPiece(state, i);
                            var destTopPieceTypeCUrrent = (destTopPieceCurrent) ? destTopPieceCurrent.type : null;

                            if (destTopPieceTypeCUrrent !== null && (destTopPieceTypeCUrrent === piece.type)) {
                                countOfSamePieces += 1;
                            }
                        }
                    }

                    //console.log('--------------------------- countOfSamePieces = ' + countOfSamePieces);
                    var destTopPiece2 = model.State.getTopPiece(state, destination);
                    var destTopPieceType2 = (destTopPiece2) ? destTopPiece2.type : null;
                    if (countOfSamePieces !== 5) {
                        if ((destTopPieceType2 === null) || (destTopPieceType2 === piece.type)) {
                            addAction(
                                model.MoveActionType.MOVE, piece, position, destination
                            );
                        }
                        ...
                        if(piece.type === 0) {
                            var countOfPassedBlackPieces = 0;
                            for(var i = 12; i <= 17; i++ ) {
                                    var destTopPieceCurrent3 = model.State.getTopPiece(state, i);
                                    var destTopPieceTypeCUrrent3 = (destTopPieceCurrent3) ? destTopPieceCurrent3.type : null;

                                    if (destTopPieceTypeCUrrent3 !== null && (destTopPieceTypeCUrrent3 !== 0)) {
                                        countOfPassedBlackPieces += 1;
                                    }
                            }

                            if(countOfPassedBlackPieces > 0 && destination >= 18 && destination <=23) {
                                if ((destTopPieceType2 === null) || (destTopPieceType2 === piece.type)) {
                                    addAction(
                                        model.MoveActionType.MOVE, piece, position, destination
                                    );
                            ...
                            var countOfPassedWhitePieces = 0;
                            for(var i = 0; i <= 5; i++ ) {
                                var destTopPieceCurrent4 = model.State.getTopPiece(state, i);
                                var destTopPieceTypeCUrrent4 = (destTopPieceCurrent4) ? destTopPieceCurrent4.type : null;

                                if (destTopPieceTypeCUrrent4 !== null && (destTopPieceTypeCUrrent4 !== 1)) {
                                    countOfPassedWhitePieces += 1;
                                }
                            }

                            if(countOfPassedWhitePieces > 0 && destination >= 6 && destination <= 11) {
                                if ((destTopPieceType2 === null) || (destTopPieceType2 === piece.type)) {
                                    addAction(
                                        model.MoveActionType.MOVE, piece, position, destination
                                    );
